...
// Cтворюємо метод accelerate для прискорення швидкості прототипу Car, збільшує this.speed на передане число та виводить рядок в консоль: Автомобіль <brand> <model> прискорився до швидкості <speed> км/год
 Car.prototype.accelerate = function(fast){
  this._speed += fast;
 console.log(`Автомобіль + ${this.brand} + ${this.model} + прискорився до швидкості + ${speed} + км/год`)
}
...
Car.prototype.brake = function(stop) {
  this._speed -= stop;
  console.log(`Автомобіль ${this.brand} ${this.model} зменшив до швидкості + ${speed} + км/год`)
}
